...
public class MiniShopItem {
	...
	public MiniShopItem(Material material, String extra, int slot, String description, int quantity, Double price, Double sellPrice) {
		...
		try {
			...
			if (price > 0D) {
				Lore.add(ChatColor.YELLOW + "Buy (Left Click) " + quantity + " @ " + VaultHelper.econ.format(price));
			}
			if (sellPrice > 0D) {
				Lore.add(ChatColor.YELLOW + "Sell (Right Click) " + quantity + " @ " + VaultHelper.econ.format(sellPrice));
			}
